...
export class DataDrivenLevel extends BaseLevel {
  constructor(game, levelNumber) {
    console.log('DataDrivenLevel constructor called for level', levelNumber);
  ...
  async initAsync() {
    ...
    if (this._initAsyncCalled) {
      console.log('initAsync already called, skipping');
      return;
    }
    ...
    try {
      ...
      this.physicsWorld = new RAPIER.default.World({ x: 0.0, y: -1.625, z: 0.0 });
      ...
      console.error(`Failed to initialize level ${this.levelNumber}:`, error);
  ...
  setupPlatforms() {
    if (!this.levelData) {
      console.warn('No level data available, creating empty level');
      return;
    }
  ...
  getPlayerSpawnPosition() {
    ...
    console.log('getPlayerSpawnPosition() returning:', position, 'levelData loaded:', !!this.levelData);
    return position;
  }
